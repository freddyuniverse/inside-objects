...
The project is built as a small, forkable experiment for people who like product design, reverse engineering, and making ordinary objects feel curious again.
...
Open the local URL printed by Vite. The interactive 3D experience runs entirely in the browser.
...
The hosted Site's `.openai/hosting.json` is deployment metadata for the original publication. If you connect this project to your own hosting setup, replace it with your own configuration rather than reusing the original project ID.
...
This repository also retains the configuration used to build the published Site with [vinext](https://github.com/cloudflare/vinext). The hosted builder runs `npm run build` against the selected source commit.
